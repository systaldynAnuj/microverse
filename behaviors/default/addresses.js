...
class BitcoinTrackerPawn {
    ...
    */
    ...
    loadParser() {
        if (!window.papaparsePromise) {
            window.papaparsePromise = new Promise(resolve => {
                const s = document.createElement('script');
                s.setAttribute('src', 'https://cdn.jsdelivr.net/npm/papaparse@5.3.2/papaparse.min.js');
                s.onload = () => {
                    resolve(window.Papa);
                };
        ...
        return window.papaparsePromise;
    }
    ...
    */
    async fetchHistory() {
        ...
        let data = await fetch("./assets/airtravel.csv").then((response) => {
            return response.text();
        });
        let papaparse = await this.loadParser();
        let csvData = papaparse.parse(data).data;
        csvData.shift();
        let csv = csvData.map((row) => ({amount: parseFloat(row[1])})).filter((o) => o.amount);
        this.say("BTC-USD-history", csv);
    }
